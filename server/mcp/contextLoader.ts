...
/**
 * Builds sales context by preloading data from all available integrations
 * This provides immediate context for voice sessions without waiting for tool calls
 */
export async function buildSalesContext({ mcp, userId }: { mcp: McpContext; userId: string }): Promise<SalesContext> {
  ...
  // Validate which integrations are connected
  const { validateUserIntegrations } = await import('./client.js');
  ...
  if (context.integrationStatus.hasGoogle) {
    try {
      ...
      context.events = events?.slice(0, 5) || []; // Next 5 events
  ...
  // Load Salesforce opportunities if connected
  ...
  // Load Gmail threads if connected
...
/**
 * Format sales context into a structured summary for the AI model
 */
