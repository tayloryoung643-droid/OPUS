          ...
          <div className="rounded-2xl border border-zinc-900/70 bg-zinc-950/60 p-5">
            ...
            : prep ? (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {/* Mutual Action Plan (mini) */}
                <Card title="Mutual Action Plan" full>
                  <div className="overflow-auto">
                    <table className="w-full text-sm">
                      <thead className="text-zinc-400">
                        <tr>
                          <th className="text-left font-normal pb-2">Milestone</th>
                          <th className="text-left font-normal pb-2">Owner</th>
                          <th className="text-left font-normal pb-2">Date</th>
                        </tr>
                      </thead>
                      <tbody className="text-zinc-300">
                        <tr className="border-t border-zinc-900/60"><td className="py-2">Kickoff</td><td>AE + Prospect</td><td>Oct 1</td></tr>
                        <tr className="border-t border-zinc-900/60"><td className="py-2">Pilot Live</td><td>RevOps</td><td>Oct 7</td></tr>
                        <tr className="border-t border-zinc-900/60"><td className="py-2">Security Review</td><td>IT/Sec</td><td>Oct 9</td></tr>
                      </tbody>
                    </table>
                  </div>
                </Card>
                ...
                <Card title="Decision Map">
                  <div className="flex flex-wrap gap-2 text-sm">
                    {[["Economic Buyer", "Unknown"], ["Champion", "Potential"], ["User", "Known"], ["Security", "Pending"]].map(([role, status], i) => (
                      <span key={i} className="px-2 py-1 rounded-full border border-zinc-800 text-zinc-300">{role}: <span className="text-zinc-400">{status}</span></span>
                    ))}
                  </div>
                </Card>

                {/* Security & Legal Checklist */}
                <Card title="Security & Legal Checklist">
                  <ul className="text-sm text-zinc-300 space-y-2">
                    {["SOC2 Type II", "DPA signed", "SSO (SAML/OIDC)", "Data flow diagram", "Least privilege scopes"].map((item, i) => (
                ...
                {/* Customer Proofs */}
                <Card title="Customer Proofs">
                  <List bullets={["Case study: Fintech reduced prep time 48%", "ROI calc: +2 meetings/week/AE", 'Quote: "Opus makes me untouchable on calls"']} />
                </Card>

                {/* Open Questions */}
                <Card title="Open Questions to Confirm">
                  <ul className="text-sm text-zinc-300 space-y-2">
                    {["Who signs the order?", "Timeline constraints?", "Security reviewer contact?", "Success criteria for pilot?"].map((q, i) => (
                      <li key={i} className="flex items-center gap-2">
                        <input type="checkbox" className="h-4 w-4 accent-purple-600" />
                        <span>{q}</span>
                      </li>
                ...
                {/* Previous Communication (read-only) */}
                <Card title="Previous Communication">
                  {prep.previousComms?.length ? (
                    <ul className="divide-y divide-zinc-900/60">
                      {prep.previousComms.map((c, i) => (
                        <li key={i} className="py-3">
                          <div className="flex justify-between text-sm">
                            <span className="text-zinc-300">{c.source}</span>
                            <span className="text-zinc-500">{c.when}</span>
                          </div>
                          <div className="text-zinc-400 text-sm">{c.summary}</div>
                        </li>
                      ))}
                    </ul>
                  ) : (
                    <div className="text-sm text-zinc-500">Connect Gmail / Salesforce / Gong to auto-summarize last touchpoints.</div>
                  )}
                </Card>

                {/* Competitor Analysis (read-only mock) */}
                <Card title="Competitor Analysis" full>
                  {prep.competitors?.length ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {prep.competitors.map((co, i) => (
                        <div key={i} className="rounded-xl border border-zinc-900/70 bg-black/50 p-4">
                          <div className="text-sm font-semibold mb-1">{co.name}</div>
                          <div className="text-xs text-zinc-400 mb-3">{co.context}</div>
                          <div className="text-xs text-zinc-400 uppercase tracking-wide mb-1">Counter</div>
                          <List bullets={co.counters} />
                          <div className="text-xs text-zinc-400 uppercase tracking-wide mt-3 mb-1">Trap</div>
                          <List bullets={co.traps} />
                          <div className="text-xs text-zinc-400 uppercase tracking-wide mt-3 mb-1">Ripcord</div>
                          <List bullets={co.ripcord} />
                        </div>
                      ))}
                    </div>
                  ) : (
                    <div className="text-sm text-zinc-500">No competitor signals yet. Add CRM tags or meeting notes to trigger battle cards.</div>
                  )}
                </Card>

                {/* Objections & Answers (editable, two-column) */}
                <Card title="Objections & Answers" full>
                  <EditableObjections items={prep.objections} onChange={(v) => { setPrep({ ...prep, objections: v }); handleSave(); }} />
                </Card>
              </div>
            ) : (
              <div className="text-zinc-400 text-sm">No prep available.</div>
            )}
          </div>
        </section>
      </main>
